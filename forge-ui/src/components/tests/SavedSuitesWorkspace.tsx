...
export type SuiteRunEligibility =
  | { kind: 'unverified' }
  | { kind: 'eligible'; source: 'authoritative_preflight'; suiteId: string; suiteRevision: number }
...
export function SavedSuitesWorkspace({ projectId, state, readRevision, refreshCurrentHead, save, runEligibility = { kind: 'unverified' } }: SavedSuitesWorkspaceProps) {
  ...
  async function open(suite: CanonicalSuiteRevision) { if (!readRevision) return; setPending(true); setMessage(null); try { setOpened(await readRevision(suite.suiteId, suite.revision)); setDraft(null) } catch { setMessage('The exact Suite revision could not be loaded. No current head was substituted.') } finally { setPending(false) } }
  async function reloadCurrent(suiteId: string) {
    ...
    try {
      ...
      setOpened(current); setDraft(null); setMessage(`Loaded authoritative current Suite revision ${current.revision}. Review it before editing again.`)
    } catch {
  ...
  async function submit() {
    ...
    setPending(true); setMessage(null)
    try { const saved = await save(request); setOpened(saved); setDraft(null); setMessage(`Saved immutable Suite revision ${saved.revision}.`) }
    catch (cause) {
  ...
  return <section aria-labelledby="saved-suites-heading" className="space-y-4 rounded-lg border border-border bg-surface p-5">
    ...
    {opened && !draft && <SuiteRevisionView suite={opened} onEdit={() => startEdit(opened)} runEligibility={runEligibility} />}
