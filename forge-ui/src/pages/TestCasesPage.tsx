...
function DefinitionDetail({ definition, project }: { definition: TestDefinitionPresentation; project: string }) {
  ...
  return <section id={id} aria-labelledby={`${id}-heading`} className="space-y-4 rounded-lg border border-border bg-elevated p-4 text-sm">
    <div className="flex flex-wrap items-start justify-between gap-3">
      ...
      <span className={`rounded-full border border-border px-3 py-1 text-xs font-semibold ${definition.schemaVersion === 2 ? 'text-brand' : 'text-flaky'}`}>{definition.schemaVersion === 2 ? 'SEALED CANONICAL SUPPORT' : 'LEGACY PROVENANCE'}</span>
    </div>
    <dl className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
      ...
      <div><dt className="text-xs uppercase text-muted">Execution eligibility</dt><dd className="text-secondary">{definition.schemaVersion === 2 ? 'Requires live preflight' : 'Unsupported for new Product execution'}</dd></div>
...
export function TestCasesPage() {
  ...
  return <div className="mx-auto w-full max-w-7xl space-y-6 px-4 py-6 sm:px-6">
    <div><h1 className="text-2xl font-semibold text-primary">Canonical Test Cases</h1><p className="mt-1 max-w-3xl text-sm text-secondary">Review sealed v2 definition authority. Historical v1 definitions remain readable only inside an explicit legacy-provenance quarantine.</p></div>
